...
class ImageDataGenerator(object):
    ...
    def _read_txt_file(self):
        ...
        with open(self.txt_file) as f:
            while 1:
                ...
                if not line:
                    break
                a = line.split('_',line.count('_'))[line.count('_')]
                b = a.split('.',a.count('.'))[0]
                self.labels.append(int(b))
                self.img_paths.append(line.rstrip('\r\n'))
                pass 
        #label = (range(self.num_classes))
        #labels = np.tile(label,int((len(self.img_paths))/64))
    ...
    def _parse_function_train(self, filename, label):
        ...
        img_resized = tf.image.resize_images(img_decoded, [31, 31])
        """
