...
class AlexNet(object):
    ...
    def create(self):
        ...
        pool1 = max_pool(norm1, 2, 2, 1, 1, padding='VALID', name='pool1')
        '''
        # 2nd Layer: Conv (w ReLu)  -> Lrn -> Pool with 2 groups
        conv2 = conv(pool1, 4, 4, 256, 1, 1, groups=2, name='conv2')
...
'''
        ...
        conv5 = conv(pool1, 2, 2, 256, 1, 1, groups=2, name='conv5')
        pool5 = max_pool(conv5, 3, 3, 1, 1, padding='VALID', name='pool5')

        # 6th Layer: Flatten -> FC (w ReLu) -> Dropout
        flattened = tf.reshape(pool5, [64, 3*3*256])
        fc6 = fc(flattened, 3*3*256, 512, name='fc6')
        dropout6 = dropout(fc6, self.KEEP_PROB)

        # 7th Layer: FC (w ReLu) -> Dropout
        fc7 = fc(dropout6, 512,512, name='fc7')
        dropout7 = dropout(fc7, self.KEEP_PROB)

        # 8th Layer: FC and return unscaled activations
        self.fc8 = fc(dropout7, 512, self.NUM_CLASSES, relu=False, name='fc8')
